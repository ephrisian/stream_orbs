...
function createSoundButton(trigger) {
  ...
  audioInput.addEventListener('change', () => {
    ...
    if (file) {
      ...
      audioInput.value = ''; // allow same file to be reselected
      saveTriggers();
  ...
  gifFileInput.addEventListener('change', () => {
    ...
    if (file) {
      ...
      gifFileInput.value = ''; // allow re-select
      saveTriggers();
  ...
  keyInput.addEventListener('click', () => {
    ...
    const keyListener = (e) => {
      ...
      if (conflict) {
        showSoundStatus(`⚠️ Key combo already used by "${conflict.label || 'unnamed'}"`, true);
      }
  ...
  volume.style.marginBottom = '5px';

  const gifPos = createGifPositionSelector(trigger);
  gifPos.style.display = 'block';
  gifPos.style.marginBottom = '5px';
  ...
  document.getElementById('soundList').appendChild(container);
}
...
function updateSoundPanel() {
  ...
  [
    'Label', 'Sound URL', 'GIF URL', 'Volume', 'Position', 'Key', '', ''
  ].forEach(text => {
  ...
  soundTriggers.forEach(createSoundButton);
}

const playingAudioMap = new Map(); // key: trigger, value: Audio element
...
function playSoundAndGif(trigger) {
  ...
  if (existing && !existing.ended && !existing.paused && !existing.seeking) {
    if (!existing._startTime || now - existing._startTime < 10000) {
      showSoundStatus("⏳ Sound already playing", true);
      return;
    }
  }

  if (!trigger.keyCombo) {
    showSoundStatus("⚠️ Trigger has no key binding", true);
  }
  ...
  if (trigger._objectUrl || trigger.soundUrl) {
    try {
      ...
      if (trigger._objectUrl) {
        const blobUrl = trigger._objectUrl;
        ...
        showSoundStatus("❌ Invalid audio URL", true);
        return;
      }
      ...
      audio.play().then(() => {
        ...
        setTimeout(() => {
          if (playingAudioMap.get(trigger) === audio) {
            playingAudioMap.delete(trigger);
          }
        }, 10000);
      }).catch(err => {
      ...
      showSoundStatus("❌ Invalid audio format", true);
    }
  } else {
    showSoundStatus("❌ No sound URL", true);
  }

  // ===== GIF logic =====
  const gifSrc = trigger._gifObjectUrl;
  if (gifSrc && gifSrc.trim()) {
    ...
    if (!stage) {
      showSoundStatus("❌ Cannot find stage element", true);
      return;
    }
    ...
    const position = trigger.gifPosition || 'bottom-left';
    applyGifPosition(img, position);
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        img.remove();
        const i = activeGIFs.indexOf(img);
...
document.addEventListener('keydown', (e) => {
  if (document.activeElement?.tagName === 'BUTTON' && document.activeElement.textContent.includes('Listening')) {
    return;
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('addSoundBtn')?.addEventListener('click', () => {
    ...
    updateSoundPanel();
    saveTriggers(); 
    showSoundStatus("✅ Added new sound trigger");
  });
