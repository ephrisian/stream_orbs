...
class WhatnotChatMonitor {
  ...
  async init() {
    // Connect to WebSocket bridge first
    this.connectToBridge();
    
    // Inject the injected script to access page context
    this.injectPageScript();
    
    // Listen for chat messages from injected script
    this.setupInjectedScriptListener();
    
    // Start WebSocket monitoring in extension context (as backup)
  ...
  injectPageScript() {
    console.log('Stream Orbs: Injecting page context script...');
    
    const script = document.createElement('script');
    script.src = chrome.runtime.getURL('injected.js');
    script.onload = () => {
      console.log('Stream Orbs: Injected script loaded successfully');
      script.remove();
    };
    script.onerror = () => {
      console.error('Stream Orbs: Failed to load injected script');
      script.remove();
    };
  ...
  setupInjectedScriptListener() {
    console.log('Stream Orbs: Setting up injected script listener...');
    
    document.addEventListener('streamOrbsChatMessage', (event) => {
      const message = event.detail;
      console.log('Stream Orbs: Received chat message from injected script:', message);
      
      // Process and send to server
      this.processChatMessage(message, 'Injected-' + message.type);
    });
  }
  ...
  connectToBridge() {
    console.log('Stream Orbs: Connecting to WebSocket bridge...');
    
    try {
      this.ws = new WebSocket(this.bridgeUrl);
      
      this.ws.onopen = () => {
        console.log('Stream Orbs: Connected to WebSocket bridge');
        this.isConnectedToBridge = true;
        this.connectionRetryCount = 0;
      ...
      this.ws.onmessage = (event) => {
        try {
          ...
          console.log('Stream Orbs: Message from bridge:', data);
          
          // Handle admin panel commands
          if (data.type === 'command' && data.target === 'extension') {
            this.handleAdminCommand(data);
          }
        } catch (error) {
          console.error('Stream Orbs: Error parsing bridge message:', error);
        }
      };
      
      this.ws.onclose = () => {
        console.log('Stream Orbs: Disconnected from WebSocket bridge');
        this.isConnectedToBridge = false;
        this.retryBridgeConnection();
      };
      
      this.ws.onerror = (error) => {
        console.error('Stream Orbs: WebSocket bridge error:', error);
        this.isConnectedToBridge = false;
      };
      
    } catch (error) {
      console.error('Stream Orbs: Failed to connect to bridge:', error);
      this.retryBridgeConnection();
    }
  }
  ...
  retryBridgeConnection() {
    if (this.connectionRetryCount >= this.maxRetries) {
      console.log('Stream Orbs: Max retry attempts reached for bridge connection');
      return;
    }
    
    this.connectionRetryCount++;
    console.log(`Stream Orbs: Retrying bridge connection in ${this.retryDelay}ms (attempt ${this.connectionRetryCount}/${this.maxRetries})`);
  ...
  sendToBridge(message) {
    if (this.ws && this.ws.readyState === WebSocket.OPEN) {
      ...
      console.warn('Stream Orbs: Bridge not connected, cannot send message:', message);
      return false;
    }
  }
  
  handleAdminCommand(data) {
    console.log('Stream Orbs: Handling admin command:', data);
    ...
    switch (data.command) {
      ...
      case 'get_status':
        this.sendToBridge({
          ...
          result: {
            ...
            uuid: this.extensionUuid,
            connected: this.isConnectedToBridge,
            url: window.location.href
          }
        });
        break;
      ...
      default:
        console.log('Stream Orbs: Unknown admin command:', data.command);
    }
  }
  
  startWebSocketMonitoring() {
    console.log('Stream Orbs: Starting WebSocket monitoring in extension context...');
    
    // This runs in extension context, so it's limited
    // The injected script will do the actual WebSocket hooking
    
    // Monitor for any WebSocket-related activity in the page
    this.monitorNetworkActivity();
  }
  
  monitorNetworkActivity() {
    console.log('Stream Orbs: Monitoring network activity...');
    
    // Check for existing WebSocket connections periodically
    this.networkCheckInterval = setInterval(() => {
      // Check for signs of WebSocket activity
      const scripts = document.querySelectorAll('script');
      let webSocketReferences = 0;
      ...
      if (webSocketReferences > 0) {
        console.log(`Stream Orbs: Found ${webSocketReferences} script(s) referencing WebSocket`);
      }
      
      // Also check for real-time elements that might indicate active connections
      const liveElements = document.querySelectorAll('[class*="live"], [class*="stream"], [class*="real"], [data-testid*="live"]');
      if (liveElements.length > 0) {
        console.log(`Stream Orbs: Found ${liveElements.length} elements indicating live content`);
      }
  ...
  processChatMessage(message, source) {
    ...
    this.sendMessageToBridge(processedMessage);
  }
  
  sendMessageToBridge(message) {
    // Send via WebSocket bridge (primary method)
    const bridgeMessage = {
      type: 'chat_message',
      target: 'admin',
      message: {
    ...
    const bridgeSent = this.sendToBridge(bridgeMessage);
    
    // Fallback to REST API if bridge is not available
    if (!bridgeSent) {
      console.log('Stream Orbs: Bridge not available, falling back to REST API');
  ...
  async sendHeartbeat() {
    try {
      // Send heartbeat via WebSocket bridge (primary)
      ...
      const bridgeSent = this.sendToBridge(bridgeHeartbeat);
      
      // Also send to server (fallback and for server tracking)
      const serverResponse = await fetch(`${this.serverUrl}/api/chat/test`, {
        method: 'POST',
        headers: {
